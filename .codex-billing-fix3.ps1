...
function To-Crlf {
    ...
    return (Normalize-Newlines $Value).Replace("`n", "`r`n")
}
...
function Replace-Exact {
    ...
    $content = Normalize-Newlines (Get-Content -Path $Path -Raw)
    $oldNormalized = Normalize-Newlines $Old
    $newNormalized = Normalize-Newlines $New
    ...
    Set-Content -Path $Path -Value (To-Crlf $content) -Encoding UTF8
}
